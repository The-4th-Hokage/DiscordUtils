...
class AutoEmbedPaginator(object):
    ...
    async def run(self, embeds, send_to=None):
        ...
        def check(reaction, user):
            return (user == wait_for and reaction.message.id == msg.id
        ...
        while True:
            ...
            if str(reaction.emoji) == self.control_emojis[0]:
                ...
                for reaction in msg.reactions:
                    try:
                        if reaction.message.author.id == self.bot.user.id:
                            await msg.remove_reaction(str(reaction.emoji),
                                                      reaction.message.author)
                    except:
                        pass
                return msg
                ...
                self.current_page = self.current_page + 1
                self.current_page = (len(self.embeds) - 1 if
            ...
            for emoji in self.control_emojis:
                if emoji == str(reaction.emoji):
                    ...
                    if cmd.lower() == "first":
                        ...
                        self.current_page = (
                            len(self.embeds) -
                            1 if self.current_page > len(self.embeds) - 1 else
                            self.current_page)
                        if self.remove_reactions:
                            try:
                                await msg.remove_reaction(
                        ...
                        self.current_page = self.current_page - 1
                        self.current_page = (0 if self.current_page < 0 else
                                             self.current_page)
                        if self.remove_reactions:
                            try:
                                await msg.remove_reaction(
                        ...
                        if something.isdigit():
                            index = int(something)
                            index = (len(self.control_emojis) -
                                     1 if index > len(self.control_emojis) - 1
                                     else index)
